---
title: 使用AEM Document Security Extension for Microsoft&reg;辦公室
...
使用Microsoft® Office的Document Security Extension，您可以執行下列工作：
...
* 開啟「文檔安全性」網頁以查看檔案的審核歷史記錄

### 連線至 Document Security 的伺服器 {#connect-to-a-document-security-server}

如果您的目的是要套用原則至檔案中，您必須要設定 Document Security 的連線設定。根據Microsoft® Office的Document Security Extension安裝方式，您可能已有預設的連線設定。 您可以為一個或多個 Document Security 的執行個體新增連線設定。您可以向 Document Security 管理員取得伺服器資訊。

將要用於保護檔案或管理受保護檔案的伺服器設定為預設伺服器。 將策略應用到新檔案或開啟「文檔安全性」網頁時，Microsoft® Office的Document Security Extension將連接到預設伺服器。 如果使用超過一個 Document Security 的執行個體，則伺服器之間切換時必須變更預設的伺服器設定。只要您獲得開啟檔案的授權，您就可以開啟任何受 Document Security 執行個體保護的檔案。

如果您的Document Security伺服器採用基於證書的身份驗證，則您需要安裝在本地電腦中收到的證書。 您必須選擇憑證驗證，並提供您要用來驗證的憑證。

在一個Microsoft® Office應用程式中配置Document Security實例的連接設定後，將為所有Word、Excel和PowerPoint配置該實例。

#### 安裝用戶端憑證 {#install-the-client-side-certificate}

如果您需要通過證書驗證或雙向驗證來訪問Document Security網頁，您將收到必須安裝在本地電腦上的證書。 您會收到一個憑證檔案 (.PFX 或 .P12 檔案) 及其密碼。
...
1. 在Microsoft® Office 2010和Office 2013的Document Security Extension中，於 **檔案安全性** 索引標籤，選取 **選擇伺服器**.
1. 按一下 **新增** 要建立連接設定，或選擇現有連接，然後按一下 **編輯**.
...
1. 按一下「**連線至此伺服器**」。Microsoft® Office的Document Security Extension會嘗試連線至您指定的伺服器。 請根據被指定的憑證，執行以下其中一項操作：
...
>[!NOTE]
>
>如果您無法連線到伺服器，請嘗試在 Internet Explorer 中開啟 Document Security 網頁。如果您無法使用Internet Explorer連接到伺服器，或者如果對話框顯示有關伺服器證書的警告，則Microsoft® Office的Document Security Extension無法連接到伺服器。 請聯絡伺服器管理員取得協助。

>[!NOTE]
如果無法連接到Document Security，則會出現一條消息，指出「用戶名和密碼不正確，請檢查配置設定，然後重試」。 如果由於其他原因無法連線，則可能會顯示此訊息。如果您是第一次連線到伺服器，請驗證您的伺服器名稱和連接埠設定是否正確。

#### 指定預設伺服器 {#specify-the-default-server}

1. 請執行下列動作：

   * 在Microsoft® Office 2010和Office 2013的Document Security Extension中，請參閱 **檔案安全性** 索引標籤，選取 **選擇伺服器**.

1. 選取要指定為預設的伺服器，然後按一下「**設定預設**」。預設伺服器旁邊會顯示星號。

### 使用協力驗證服務提供者 {#using-third-party-authentication-providers}

您可以使用有 AEM Forms Document Security 的協力驗證服務提供者。這些身份驗證提供程式可幫助您為受保護的文檔添加額外的訪問層。 AEM Forms Document Security 可支援以下的延伸驗證工作流程：
...
您可以將自訂驗證頁面封裝為war檔案，並將war檔案部署至AEM Forms伺服器。 war檔案包含接受用戶憑據和針對AEM Forms伺服器進行身份驗證的完整邏輯。 AEM Forms Document Security 有以下對自訂驗證頁面的規定：
...
1. 在輸入後，將以下條目添加到SSO節點下的config.xml檔案中 *&lt;node name=&quot;AllowedUrls&quot;>*:
...
#### 使用在AEM Forms伺服器上設定的第三方身分提供者，進行預設延伸驗證工作流程 {#default-extended-authentication-workflow-with-third-party-identity-providers-configured-on-aem-forms-server}

擴充驗證可使用AEM Forms伺服器上可用的不同驗證類型。 例如，SAML，[還有什麼例子]。

注意：如果已在AEM Forms伺服器上設定SAML提供者，則在顯示登錄URL之前，會顯示包含為SAML驗證設定之所有身分提供者的頁面。

在 Acrobat 開啟受保護文件時，系統將顯示以下畫面。

#### 在AEM Forms伺服器上設定SAML提供者時的自訂延伸驗證工作流程 {#custom-extended-authentication-workflow-when-saml-providers-are-configured-on-aem-forms-server}

如果已在AEM Forms伺服器上設定SAML提供者，則在顯示登錄URL之前，會顯示包含為SAML驗證設定之所有身分提供者的頁面。

在AEM Forms伺服器上設定SAML提供者時，設定自訂延伸驗證工作流程的權限為：

* SAML驗證是在AEM Forms伺服器上設定
* 自訂戰爭(包含自訂驗證頁面和完整邏輯，以接受使用者憑證和針對AEM Forms伺服器進行驗證)會部署至AEM Forms伺服器。

#### 使用自訂頁面列出 SAML 驗證 {#using-custom-page-for-listing-saml-authentications}

您也可以顯示自訂頁面，納入AEM Forms伺服器上設定的所有驗證提供者。 要建立這樣的頁面：

1. 將自訂驗證頁面封裝為war檔案，並將war檔案部署至AEM Forms伺服器。 war檔案包含接受用戶憑據和針對AEM Forms伺服器進行身份驗證的完整邏輯。
...
若要了解建立和管理原則的更多資訊，請參閱 [Document Security 用戶說明](https://help.adobe.com/en_US/AEMForms/6.1/RMHelp/)。
...
>[!NOTE]
您只能將原則套用至Word檔案(Microsoft® Office 2010和2013中的.doc、.docx和.docm)、Excel活頁簿檔案(Microsoft® Office 2010和2013中的.xls、.xlsx和.xlsm)，以及Microsoft® Office 2010和2013中的PowerPoint簡報檔案(.ppt、.pptx和.ptm)。 您不能將原則套用在 Word 範本檔案 (.dot)、Excel 範本檔案 (.xlt) 和 PowerPoint 範本檔案 (.pot)。

#### 套用一項原則 {#apply-a-policy}

1. 在Microsoft® Office 2010和2013的Document Security Extension中，請參閱 **檔案安全性** 索引標籤，選取 **安全>選擇策略**.
...
1. 在Microsoft® Office 2010和2013的Document Security Extension中，於 **檔案安全性** 頁簽，選擇**安全> ***[策略名稱]*.
1. 儲存檔案。
...
無論您是檔案發佈者組織的內部還是外部，都可以使用受策略保護的檔案。 若要開啟受原則保護的檔案，您必須獲得 Document Security 認可，方法是透過連結的 LDAP 或 Active Directory 清單內容被新增為 JEE 上的 LiveCycle 或 AEM Forms 本機用戶，或在被邀請後向註冊 Document Security 的用戶。

如果您收到受原則保護的檔案，且沒有 Document Security 帳戶，或者如果您收到註冊邀請，請與向您發送文件的人聯絡以尋求幫助。

### 在Microsoft® Office中使用受策略保護的檔案 {#working-with-policy-protected-files-in-microsoft-office}

Microsoft® Office的Document Security Extension會限制某些Word、Excel和PowerPoint功能，以保護檔案發佈者的智慧財產權。 如果您沒有變更檔案的權限，您就無法儲存修改內容。
...
如果有某個功能無法使用，則選單上的命令名稱和相關的工具欄按鈕將不適用。在Microsoft® Office的Document Security Extension中，當您將滑鼠指標暫留在命令或按鈕上時，工具提示會指出檔案安全性無法使用該命令。
...
從SharePoint伺服器開啟Microsoft® Office的Document Security Extension中受保護的檔案時，請確定與檔案類型(例如Microsoft® Word、Microsoft® Excel或Microsoft® PowerPoint)相關聯的Microsoft® Office程式已開啟。 如果要試著在不開啟相關應用程式的情况下開啟檔案，檔案可能無法開啟，且系統會顯示錯誤訊息並指出您必須安裝適用的外掛程式。除了開啟所需的應用程式外，還建議您先清除快取資料夾，再從SharePoint伺服器開啟Microsoft® Office的Document Security Extension中的受保護檔案。 此外，當您從 SharePoint 伺服器開啟受保護的文件時，不論是否套用原則，文件上所有權限皆為停用狀態。
...
1. 在Microsoft® Office 2010和2013的Document Security Extension中，於 **檔案安全性** 索引標籤，選取 **移除**.
...
在Microsoft® Office 2010的Document Security Extension中，「Document Security」（文檔安全性）頁簽上的「Security Status」（安全狀態）組將顯示您對檔案的權限。

請執行下列動作：

* 在Microsoft® Office 2010和2013的Document Security Extension中，於 **「文檔安全性」頁簽**，在 **安全狀態** 組，按一下任何項。

### 當啟用自動套用原則時儲存文件 {#saving-documents-when-auto-apply-policy-is-enabled}

如果管理員啟用了「自動應用」策略功能，則在保存文檔時，您建立或編輯的任何文檔都會受到自動保護。

如果啟用了「自動應用」策略，Document Security Extension for Microsoft® Office將提示您登錄到Document Security伺服器。 您必須提供您的使用者名稱和密碼，以供伺服器驗證。 如果您提供了正確的登錄憑據，則文檔將被保存和保護。
...
* 在Microsoft® Office 2010和2013的Document Security Extension中，於 **檔案安全性** 索引標籤，選取 **離線同步**.

   ***注意**：即使用戶沒有文件的離線權限，也可以使用「離線同步處理」按鈕。但是，選取按鈕不會起任何作用。*

### 使用動態浮水印 {#working-with-dynamic-watermarks}

Microsoft® Office的Document Security Extension支援在受原則保護的檔案中加入動態文字水印。 動態浮水印可以包含可能變更的資訊，例如日期、時間、用戶名稱或原則名稱。如果用戶列印受原則保護的檔案，且該檔案包含動態浮水印和列印權限，則浮水印將顯示在輸出中。

檔案 Document Security Extension 不支援豐富的浮水印功能，例如 PDF 式浮水印、浮水印中的多個元素、文字格式選項和頁面範圍。

您可以使用 Document Security 網頁建立動態浮水印。若要更多了解如何在受原則保護文件中建立並納人動態浮水印，請參閱 [Document Security 用戶說明](https://www.adobe.com/go/learn_lc_euRightsMgmt_11)。

Microsoft® Office的Document Security Extension支援下列浮水印功能：
...
若要開啟「Document Security」網頁，請在Document Security Extension for Microsoft® Office 2010和2013中，於 **檔案安全性** 索引標籤，選取 **建立和管理策略**. 如果您尚未提供登入資訊，瀏覽器會開啟伺服器登入頁面。
...
1. 請執行下列動作：
   ...
   * 在Microsoft® Office 2010或2013的Document Security Extension中，請於 **檔案安全性** 索引標籤，選取 **安全>更改安全**.
...
1. 請執行下列動作：
   ...
   * 在Microsoft® Office 2010和2013的Document Security Extension中，於 **檔案安全性** 索引標籤，選取 **撤銷**.

   Document Security 網頁會開啟至撤銷文件頁面。

1. 指定要顯示的訊息，並指定更新版的 URL (若有)，然後按一下「**確定**」。

若要了解撤銷檔案存取權限的更多資訊，請參閱 [Document Security 用戶說明](https://help.adobe.com/en_US/AEMForms/6.1/RMHelp/)。
...
在Microsoft® Office 2010和2013的Document Security Extension中，於 **檔案安全性** 索引標籤，選取 **審核歷史記錄**.

Document Security 網頁會開啟至活動頁面，並顯示目前檔案的已審核活動。

### Microsoft® Office受限功能 {#microsoft-office-restricted-features}

為保護您的智慧財產權，當受策略保護的檔案開啟時，某些Microsoft® Office功能將不可用。 不適用功能清單取決於目前用戶獲得的權限。有些功能僅不適用於受保護的檔案；而當您在受保護工作階段時，其他功能則不適用所有檔案。通常，從您開啟受原則保護的檔案起，您都是在受保護工作階段中，直到您關閉應用程式或工作階段到期為止。
...
>[!NOTE]
若套用原則的檔案中含有前往嵌入式檔案的連結時，該原則不會套用在所連結的檔案中。Microsoft® Office的檔案安全性不會將保護延伸至連結的檔案。

* 受原則保護的 Word、Excel 和 PowerPoint 檔案會被封鎖，而無法在 Internet Explorer 瀏覽器視窗中開啟。
* 如果用戶只獲得「變更」權限，他們法使用 Windows 剪貼簿將內容複製至其他應用程式的檔案。使用者可以啟用Microsoft® Office剪貼簿選項，將內容複製到檔案。
* 在Microsoft® Office中開啟受策略保護的檔案，會使打印螢幕密鑰在關閉應用程式或會話過期之前不可用。
* Microsoft® Office的檔案安全性不支援基於Web的分佈式創作和版本設定(WebDAV)。 通常，您無法從WebDAV資料夾開啟受策略保護的檔案。 如果您可以開啟受原則保護的檔案，您沒有儲存、列印、變更或複製該檔案的權限。

適用於受策略保護檔案的一般安全性包括以下限制：

在受保護的工作階段中，Word、Excel 和 PowerPoint 許多常見功能可能會受到限制。

如果不允許用戶對其進行更改的受策略保護的檔案處於開啟狀態，則以任何方式更改該檔案的命令將不可用。 只有開啟或建立文檔和更改應用程式首選項的命令才可用。
...
***附註&#x200B;**:從2010 Microsoft® Office系統版本的Word、Excel和PowerPoint啟動工作流的功能僅在Office Professional Plus 2010、Office Enterprise 2010和Office Ultimate 2010套件以及這些計畫的獨立2010年Office版本中提供。*
...
**首頁 > 剪貼簿 > 複製** 受到複製權限的限制。如果不允許複製，則複製的內容無法貼到任何其他檔案或Microsoft® Office剪貼簿中。 如果用戶擁有變更權限，則可以在受保護檔案中複製內容。
...
如需JEE術語上的LiveCycle和AEM表單的相關資訊，請參閱 [第19章：字彙表](https://www.adobe.com/go/learn_aemforms_designer_65).
